...
为了提高系统安全性并遵循最小权限原则，我们已删除所有初始测试用户（包括"anon"、"tomori"、"rana"、"soyo"和"taki"）。这些用户在之前的版本中被自动创建，现在系统启动时不会创建任何默认用户，所有用户都需要通过正常注册流程创建。

默认服务器和群组结构仍会被创建，第一个注册的用户将自动成为默认服务器的所有者。这种变更增强了系统安全性，避免了预设凭据可能带来的风险。 